...
func (f *fakeFetcher) Fetch(_ context.Context, ref string) (io.ReadCloser, *v1.Config, error) {
	f.fetchCount.Add(1)
	if f.fetchHold != nil {
		<-f.fetchHold
	}
	...
	f.mu.Unlock()
	if !ok {
		return nil, nil, errors.New("no tar for " + ref)
	}
	return io.NopCloser(strings.NewReader(body)), &v1.Config{}, nil
}
...
func TestEnsure_ExtractsOnceThenServesFromCache(t *testing.T) {
	...
	digest, lower, _, err := c.Ensure(t.Context(), "img:v1")
	if err != nil {
	...
	if _, _, _, err := c.Ensure(t.Context(), "img:v1"); err != nil {
...
func TestEnsure_SingleflightCollapsesConcurrentExtracts(t *testing.T) {
	...
	for i := 0; i < callers; i++ {
		go func(i int) {
			defer wg.Done()
			_, _, _, errs[i] = c.Ensure(t.Context(), "img:v1")
		}(i)
	}
