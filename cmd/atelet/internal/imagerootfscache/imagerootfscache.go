...
// imageCfgFileName is the name of the persisted image config JSON
// inside each cache entry directory. Written on cold extract, read on
// warm hits, so callers get the OCI image config alongside the
// lowerDir without a second registry round trip.
const imageCfgFileName = "image-config.json"
...
type Fetcher interface {
	...
	Fetch(ctx context.Context, ref string) (io.ReadCloser, *v1.Config, error)
}
...
// cache if not already present, and returns the digest, the absolute
// lowerDir path that overlayfs should stack on top of, and the OCI
// image config persisted alongside the rootfs.
//
...
// leader's result.
func (c *Cache) Ensure(ctx context.Context, ref string) (digest, lowerDir string, imageCfg *v1.Config, err error) {
	digest, err = c.fetcher.ResolveDigest(ctx, ref)
	if err != nil {
		return "", "", nil, fmt.Errorf("while resolving digest for %q: %w", ref, err)
	}
	...
	if err != nil {
		return "", "", nil, fmt.Errorf("while extracting %q (digest %s): %w", ref, digest, err)
	}
	cfg, cfgErr := readImageCfg(entryDir)
	if cfgErr != nil {
		return "", "", nil, fmt.Errorf("while reading image config for %q: %w", ref, cfgErr)
	}
	return digest, lowerDir, cfg, nil
}

func readImageCfg(entryDir string) (*v1.Config, error) {
	data, err := os.ReadFile(filepath.Join(entryDir, imageCfgFileName))
	if err != nil {
		return nil, err
	}
	var cfg v1.Config
...
func (c *Cache) extractLocked(ctx context.Context, ref, digest string) error {
	...
	tarData, imageCfg, err := c.fetcher.Fetch(ctx, ref)
	if err != nil {
	...
	// Persist the image config next to the rootfs so warm-path
	// Ensure calls can return it without a second registry round
	// trip. Write before the ready marker so a crash between the
	// two just re-triggers the cold path.
	if imageCfg != nil {
		cfgBytes, err := json.Marshal(imageCfg)
		if err != nil {
			return fmt.Errorf("marshaling image config: %w", err)
		}
		if err := os.WriteFile(filepath.Join(entryDir, imageCfgFileName), cfgBytes, 0o600); err != nil {
			return fmt.Errorf("writing image config: %w", err)
		}
	}
