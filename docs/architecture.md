...
The node-level subsystem manages the physical execution of sandboxes and the movement of snapshots.
  ...
  * **Lifecycle Management**: The `ateom` process invokes the sandbox runtime (e.g., `runsc` for gVisor) to checkpoint or restore processes within the physical pod boundaries. To support seamless resumption of network-connected agents, Substrate invokes `runsc` with the `--allow-connected-on-save=true` flag, allowing snapshots to be taken even with active TCP/Unix domain socket state.
...
Triggered by an inactivity timeout or an explicit `SuspendActor` call.
