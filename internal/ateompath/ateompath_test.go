...
// (namespace:name:id) routinely contains ':', so the upper/work
// helpers must NOT derive from ActorPath; this test would have
// caught the earlier two regressions immediately.
func TestOverlayOptionPathsHaveNoReservedColon(t *testing.T) {
	// Realistic input with colons throughout the actor key.
	ns := "rl-poc"
	name := "rl-poc-1"
	id := "bcb1e411-158e-41e3-9103-2dede2d2b8e5"
	container := "pause"
	...
	for _, tc := range []struct {
		...
		{"upperDir", OCIBundleUpperDir(ns, name, id, container)},
		{"workDir", OCIBundleWorkDir(ns, name, id, container)},
	} {
